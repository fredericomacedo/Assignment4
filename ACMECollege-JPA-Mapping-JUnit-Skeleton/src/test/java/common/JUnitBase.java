...
public class JUnitBase {
	...
	protected static void deleteAllData() {
	    ...
	    try {
	        ...
	        // Delete all rows from the entities. The order matters.
	        ...
	        // If an error occurs, we roll back the changes.
	...
	public static <T> int deleteAllFrom(Class<T> entityType, EntityManager em) {
	    ...
	    delete.where(cb.conjunction()); // Optional: Adds a "where 1=1" clause, might be necessary for some JPA providers to execute a delete without a where clause
